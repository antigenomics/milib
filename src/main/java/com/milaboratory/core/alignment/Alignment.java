...
public final class Alignment<S extends Sequence<S>> implements java.io.Serializable {
    /**
     * Initial sequence. (upper sequence in alignment; sequence1; reference)
     */
    final S sequence1;
    /**
     * Mutations
    ...
    /**
     * Range in initial sequence (sequence1; reference)
     */
    final Range sequence1Range;
    /**
     * Range in mutated sequence (sequence2; query)
     */
     ...
     *
     * @param full if true, skip operations to establish global query/reference positioning will be added
     * @return CIGAR string representation of this alignment
     */
    public String getCigarString(boolean full) {
        StringBuilder sb = new StringBuilder();
        for (AlignmentElement ae : getAlignmentElements(full)) {
     ...
     */
    public List<AlignmentElement> getAlignmentElements(boolean full) {
        List<AlignmentElement> result = new ArrayList<>();

        // adding skip elements
        if (sequence2Range.getLower() > 0)
            result.add(new AlignmentElement(
                    new Range(0, 0), new Range(0, sequence2Range.getLower()), false));
        if (sequence1Range.getLower() > 0)
            result.add(new AlignmentElement(
                    new Range(0, sequence1Range.getLower()),
                    new Range(sequence2Range.getLower(), sequence2Range.getLower()), false));
        ...
        while (true) {
            ...
            if (currentMutationIndex == mutations.size()) { // all mutations processed
                mutation = NON_MUTATION;
                newSeq1Position = sequence1Range.getTo();
            } else { // ingesting next mutation
                mutation = mutations.getMutation(currentMutationIndex);
                newSeq1Position = getPosition(mutation);
            }

            matchedLetters = newSeq1Position - lastSeq1Position;
            newSeq2Position = lastSeq2Position + matchedLetters;

            if (matchedLetters > 0) { // there are matching letters between current and previous mutations
                result.add(new AlignmentElement(
            ...
            if (mutation == NON_MUTATION) {
                assert newSeq2Position == sequence2Range.getTo();
                break;
            }
            ...
            if (isSubstitution(mutation)) {
                // determine the last adjacent substitution in the group
                do {
                    ++newSeq1Position;
                    ++newSeq2Position;
                } while (++currentMutationIndex != mutations.size() &&
                        isSubstitution(mutation = mutations.getMutation(currentMutationIndex)) &&
                        getPosition(mutation) == newSeq1Position);

                // previous loop always overruns by one letter
                --currentMutationIndex;

                // adding mismatch group
                result.add(new AlignmentElement(
                        new Range(lastSeq1Position, newSeq1Position),
                        new Range(lastSeq2Position, newSeq2Position),
                        false));
                ...
                // determine the last adjacent insertion in the group
                do {
                    ++newSeq2Position;
                } while (++currentMutationIndex != mutations.size() &&
                        isInsertion(mutation = mutations.getMutation(currentMutationIndex)) &&
                        getPosition(mutation) == newSeq1Position);
                ...
                // determine the last deletion substitution in the group
                do {
                    ++newSeq1Position;
                } while (++currentMutationIndex != mutations.size() &&
                        isDeletion(mutation = mutations.getMutation(currentMutationIndex)) &&
                        getPosition(mutation) == newSeq1Position);
