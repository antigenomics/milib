...
public final class AlignmentElement {
    ...
    public AlignmentElement(Range referenceRange, Range queryRange, boolean isMatch) {
        ...
        if (referenceRange.length() == 0 && queryRange.length() == 0)
            throw new IllegalArgumentException();
    ...
    public AlignmentElementType getType() {
        if (referenceRange.isEmpty())
            return match
                    ? AlignmentElementType.Deletion
                    : AlignmentElementType.SkippedQuery;
        if (queryRange.isEmpty())
            return match
                    ? AlignmentElementType.Insertion
                    : AlignmentElementType.SkippedReference;
        return match
                ? AlignmentElementType.Match
                : AlignmentElementType.Mismatch;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(referenceRange, queryRange, match);
    }
